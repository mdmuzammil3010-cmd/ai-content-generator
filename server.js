...
function formatContent(rawText, keywords) {
  // Split by sentences for bullets
...
// Generate text using GPT4All Local API (formatted)
app.post("/api/generate-text", async (req, res) => {
  try {
    const { topic, tone, platform, length, keywords } = req.body;

    // Build the prompt requesting proper formatting
    const prompt = `(${platform || "Any platform"}) ${
      tone ? tone + ":" : ""
...
- Add 3-5 relevant hashtags at the bottom`;
    ...
    // Fetch available models from GPT4All local API
    ...
    // Format content with bullets and hashtags
    ...
    console.error(err);
    res.status(500).json({ error: "GPT4All API call failed", details: err.message });
  }
});
